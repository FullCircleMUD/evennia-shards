...
- **[shard-isolation.md](shard-isolation.md)** — How the library enforces the partition between shards at the Django/Evennia level: four chokepoints (`from_db` override, `pre_save`/`pre_delete` signals, `QuerySet.update()` override) that together prevent cross-shard reads, writes, and deletes without a broad manager filter or idmapper modification. *Being superseded on the `django-multitenant-trial` branch — see [tenancy.md](tenancy.md).*
- **[tenancy.md](tenancy.md)** *(branch: django-multitenant-trial)* — Replacement for the chokepoint design: tenant-context-driven auto-filtering at the SQL layer via django-multitenant. Documents the installation process (`bootstrap_tenant_context()` + `install_tenancy_on_objectdb()`), the two-element tenant-list trick that gives global `"*"` rows visible from every shard, and the integration choices (wrap vs copy, patch-in-place vs swap, signature-tolerant `_do_update`).

...
The handoff window is the same shape as misroute. During `cross_shard_character_move`, the atomic block flips `shard_id` and `db_location_id` together, then evicts the row from the source idmapper before redirecting sessions. Messages arriving at the **source** during that window — from a sender whose view of `shard_id` was stale — trip the chokepoint and defer (eventually replying `undeliverable_reply`). Messages arriving at the **target** between the DB write and the player's session reattaching look up the row successfully but find `obj.sessions.all()` empty, so `Object.msg` silently no-ops — the same behaviour as messaging any unpuppeted object in vanilla Evennia. There is no dedicated "in-transit" state machine: the chokepoints, the sender-side `.values_list` shard lookup, and Evennia's own per-session fanout converge on correctness without one.
...
Specialised cross-shard interactions (tells, channel propagation, scry-style effects) are **out of scope for MVP** — flagged as future work, candidates for a `evennia_shards/contrib/` layer (analogous to `evennia/contrib/`). Contrib modules would give consumers a starter implementation to use, extend, or replace, while keeping the library core principle-3 clean: nothing in the core layer mandates a game concept, contrib offers conventions for the common cases. Room broadcast is **not** in scope at all: a room and its contents share a single `shard_id` by construction (otherwise the `from_db` chokepoint would refuse the location FK), so `Object.msg_contents` is intrinsically local and uses vanilla Evennia messaging without any bus involvement.
